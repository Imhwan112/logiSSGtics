...
//    }
    ...
    public void approveUser(User user) {
        String sql = "UPDATE logissgtics.user SET status = \"activate\" where user_id = ?";
        try (
                Connection conn = dataSource.getConnection();
                PreparedStatement pstmt = conn.prepareStatement(sql);
        ) {
            ...
            pstmt.executeUpdate();
            System.out.println("회원가입 요청이 승인 되었습니다.");
        } catch (SQLException e) {
            System.out.println("아이디 체크 실패햇네요?");
            e.printStackTrace();
        } catch (Exception e) {
...
//                System.out.println("값이 있다!");
    ...
    public Customer authCustomer(String user_id, String password) {
        String sql = "SELECT * FROM logissgtics.Customer WHERE user_id = ? AND password = ?";
...
//                System.out.println("값이 있다!");
                // customer
                Customer user = storeCustomer(rs);
                System.out.println(
                        "아이디 비밀번호 인증 완료!\n"
                                + "아이디: " + user.getUser_id()
                                + " 비빌번호: " + user.getPassword()
    ...
    public User storeUser(ResultSet rs) {
        ...
        try {
            ...
            user.setAddr2(rs.getString("addr2") == null? 0 : rs.getString("addr2").charAt(0));
    ...
    public Customer storeCustomer(ResultSet rs) {
